...
// Generate JWT (Jitsi token) modify
async function generateJWT(
  ...
  try {
    ...
    // Call async createContext with room
    const context = await createContext(userInfo, room);
    const payload = {
      aud: JWT_APP_ID,
      iss: JWT_APP_ID,
      sub: sub,
    ...
    console.log('generateJWT: Payload before signing:', JSON.stringify(payload, null, 2));

    const jwt = await create(header, payload, cryptoKey);
    console.log('generateJWT: Generated token:', jwt);
...
// -----------------------------------------------------------------------------
// Generate JWT (Jitsi token) - default
// -----------------------------------------------------------------------------
//async function generateJWT(
//  userInfo: Record<string, unknown>,
//  sub: string,
