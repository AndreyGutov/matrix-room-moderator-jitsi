-- Token moderation
-- This module looks for a field in incoming JWT tokens called "context.user.moderator".
...
module:hook("muc-room-created", function(event)
        ...
        room.set_affiliation = function(room, actor, jid, affiliation, reason)
                ...
                -- no one else can assign owner (in order to block prosody/jitsi's built-in moderation functionality)
                elseif affiliation == "owner" then
                        return nil, "modify", "not-acceptable"
                -- keep other affiliation-based features working as normal (hopefully, haven't needed to use/test any of it)
                else
...
function setupAffiliation(room, origin, stanza)
        if origin.auth_token then
                log("info", "Received token for user: " .. stanza.attr.from);
                log("debug", "Raw token: " .. origin.auth_token);
                ...
                if dotFirst then
                        ...
                        if dotSecond then
                                ...
                                log("debug", "Token body (base64): " .. bodyB64);
                                local success, body = pcall(json.decode, basexx.from_url64(bodyB64));
                                if success then
                                        log("info", "Decoded token body: " .. json.encode(body));
                                        local jid = jid_bare(stanza.attr.from);
                                        -- Check for context.user.moderator
                                        local isModerator = false
                                        if body["context"] and body["context"]["user"] and body["context"]["user"]["moderator"] then
                                                isModerator = body["context"]["user"]["moderator"]
                                                log("info", "Moderator status from token: " .. tostring(isModerator))
                                        else
                                                log("warn", "No 'context.user.moderator' found in token for user: " .. jid)
                                        end
                                        if isModerator or is_admin(jid) then
                                                log("info", "Setting affiliation to owner for " .. jid)
                                                room:set_affiliation("token_plugin", jid, "owner")
                                        else
                                                log("info", "Setting affiliation to member for " .. jid)
                                                room:set_affiliation("token_plugin", jid, "member")
                                        end
                                else
                                        log("error", "Failed to decode token body for user: " .. jid .. ", error: " .. tostring(body))
                                end
                        else
                                log("error", "Invalid token format (no second dot) for user: " .. jid)
                        end
                else
                        log("error", "Invalid token format (no first dot) for user: " .. jid)
                end
        else
                log("warn", "No auth_token for user: " .. stanza.attr.from)
        end
end;
